...
def create_prompt_from_component(prompt_component) -> ChatPromptTemplate:
    """
    ...
    try:
        ...
        logger.info(f"========={template_content}=========")
        # chat_history는 사용하지 않으므로 단일 human 템플릿만 사용
...
class A2APlannerComponent(LCAgentComponent):
    ...
    async def create_work_plan(self, available_agents: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            # chat_history 미사용: 빈 리스트로 고정
            langchain_chat_history = []

            logger.info(f"user_task: {user_task}----------------------------------")
